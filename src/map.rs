...
impl Map {
    ...
    pub fn new(root_path: &Path) -> Result<Self, MapError> {
        ...
        let provinces = Self::load_image(root_path, &default_map.provinces)?;
        let terrain = Self::load_image(root_path, &default_map.terrain)?;
        let rivers = Self::load_image(root_path, &default_map.rivers)?;
        let heightmap = Self::load_image(root_path, &default_map.heightmap)?;
        let trees = Self::load_image(root_path, &default_map.tree_definition)?;
        let normal_map = Self::load_image(root_path, Path::new("world_normal.bmp"))?;
        let cities_map = Self::load_image(root_path, Path::new("cities.bmp"))?;
    ...
    fn verify_images(
        ...
        let heightmap_aspect_ratio = heightmap.width() as f64 / heightmap.height() as f64;
        let trees_aspect_ratio = trees.width() as f64 / trees.height() as f64;
        ...
        let normal_aspect_ratio = normal_map.width() as f64 / normal_map.height() as f64;
